...
public String[] findWords(String[] words) {
  ...
  public String checkeach(String current){
	  List<Character>  first = Arrays.asList('Q','W','E','R','T','Y','U','I','O','P','q','w','e','r','t','y','u','i','o','p');
	  List<Character>  second = Arrays.asList('a','s','d','f','g','h','j','k','l','A','S','D','F','G','H','J','K','L');
	  List<Character>  third = Arrays.asList('Z','X','C','V','B','N','M','z','x','c','v','b','n','m');
       
        char[] req = current.toCharArray();
       
        List<Character> range = null;
        if(first.contains(req[0])) range= first;
        else if(second.contains(req[0])) range = second;
        else if(third.contains(req[0])) range = third;
